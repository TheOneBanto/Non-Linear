...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	public int my_count_at_level(int level) {
		...
		try {
			int res = 0;
			if (this.my_is_empty()) {// if the current node is empty return res
				return res;
			}
			// If levels start at 0, change this to 0.
			// If levels start at 1, change this to 1.
			// If the amount of levels left is equal to the first level on the tree
			// increment res.
			if (level == 1) {
				res += 1;
			}
			// Else check the children of the current node.
			else {
				res += this.my_left_tree().my_count_at_level(level - 1);// check the node left of the tree
				res += this.my_right_tree().my_count_at_level(level - 1);// check the node right of the tree
			}
			return res;
		} catch (Exception e) {
	...
	public boolean my_is_balanced() {
		...
		// TO-DO
		boolean res;
		// If the root node is empty rhe tree is balanced
		if (my_is_empty()) {
			res = true;
		}
		// Else we check the length of the left tree against the length of the right
		// tree and see if difference in length is within one away from each other
		else {
			if ((this.my_left_tree().my_length() - this.my_right_tree().my_length()) < 2) {
				// If there is a difference within one away of the two lengths we chech the left
				// and right nodes
				res = this.my_left_tree().my_is_balanced();
				res = this.my_right_tree().my_is_balanced();
			} else {
				// Else the tree is imballanced
				res = false;
			}
		}
		return res;
	...
	public int my_count_smaller_nodes(T1 key) {
		try {
			// TO-DO
			int res = 0;
			// If the tree is not empty we continue
			if (!my_is_empty()) {
				// If the difference between the root key and the key we are checking for is
				// less then 0 we increment res
				if (this.root.getKey().compareTo(key) < 0) {
					res += 1;
				}
				// Now we check all the nodes to the left and right of the root node at level
				// 0/1
	...
	public int my_find_node_at_level(T1 key) {
		...
		try {
			int res = 0;
			// Compares the root key to the key that we want to check against
			// If the key is smaller than the root key then we go left.
			if (key.compareTo(this.root.getKey()) < 0) {
				// go left of tree
				res += this.my_left_tree().my_find_node_at_level(key);
			}
			// If the key is equal to the root key we return 0
			else if (key.compareTo(root.getKey()) == 0) {
				return 0;
			}
			// If the key is greater than the root key then we go right.
			else {
				// go right of tree
			...
			// TODO: handle exception
			System.err.println(e + ": No key " + key);
			return -1;
		}
	}
